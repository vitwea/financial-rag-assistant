...
def load_index() -> tuple[faiss.IndexFlatIP, list[dict], BM25Okapi]:
    ...
    logger.info("FAISS index: %d vectors | BM25 index: %d chunks", index.ntotal, len(metadata))
...
def faiss_search(
    ...
    logger.debug("FAISS: %d candidates (company=%s year=%s)", len(results), company_filter, year_filter)
    return results
...
def reciprocal_rank_fusion(
    ...
    merged = []
    for uid, score in sorted(rrf_scores.items(), key=lambda x: x[1], reverse=True):
...
def _detect_companies(query: str) -> list[str]:
    ...
    return found if found else ALL_COMPANIES
...
def retrieve(
    ...
    """
    logger.info("Retrieving: \"%s\" (company=%s year=%s)", query, company_filter, year_filter)

    if company_filter or year_filter:
        expanded   = expand_query(query, company_filter)
        candidates = hybrid_search(expanded, index, metadata, bm25,
                                   top_k=FAISS_TOP_K,
    ...
    if _is_comparison_query(query):
        companies  = _detect_companies(query)
        candidates = hybrid_search_per_entity(
            query, index, metadata, bm25, target_companies=companies,
        )
        scored = rerank_all(query, candidates)
