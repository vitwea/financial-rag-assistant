...
FAISS_TOP_K = 20
BM25_TOP_K = 20
RRF_K = 60  # higher → smoother fusion, lower → rank difference matters more
RERANK_TOP_K = 5
FAISS_PER_ENTITY = 8
MIN_PER_ENTITY = 1
...
def reciprocal_rank_fusion(
    ...
    Rank-based (not score-based) — robust to scale differences between
...
def rerank_all(query: str, candidates: list[dict]) -> list[dict]:
    ...
    if not api_key:
        ...
        return sorted(
            candidates,
            key=lambda c: c.get("rrf_score", c.get("faiss_score", 0)),
            reverse=True,
        )
...
def balanced_select(
    ...
    # Completar hasta top_k con los mejores restantes
    for chunk in scored:
...
def _detect_companies(query: str) -> list[str]:
    ...
    Importante: ya no hay fallback a ALL_COMPANIES.
    Si el usuario no menciona empresas, devolvemos [] y dejamos
    que el routing trate la consulta como global/normal.
    """
    low = query.lower()
...
def retrieve(
    ...
    Note: signature now includes `bm25` as third positional argument.
    """
    logger.info(
    ...
    #2) Temporary queries (trends, developments, etc.)
    ...
    if _is_comparison_query(query):
        ...
        # If no companies are mentioned, we do not force artificial comparisons:
        # we treat the query as a global standard.
        if not companies:
            candidates = hybrid_search(
                query,
                index,
        ...
        # Actual comparison between the companies mentioned
        candidates = hybrid_search_per_entity(
            query,
            index,
            metadata,
    ...
    #4) Standard query (no filters, no comparison, no time frame)
    candidates = hybrid_search(
        query,
        index,
        metadata,
